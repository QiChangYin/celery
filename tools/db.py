...
class Db:
    ...
    def get_tasksinfo(self):
        ...
        query_str = "select b.id as id,b.task_id as task_id,b.first as first,b.second as second,b.log_date as logdate,a.status as status,a.result as result,a.traceback as traceback " \
                    "from celery_taskmeta a inner join test_celery_plus b on a.task_id=b.task_id;"
        cur.execute(query_str)
        rows  = cur.fetchall()
        cur.close()
        return rows

    def get_MultiTasksinfo(self):
        cur = self.con.cursor(DictCursor)
        query_str = "select b.id as id,b.task_id as task_id,b.multiplier as multiplier,b.multiplicand as multiplicand,b.log_date as logdate,a.status as status,a.result as result,a.traceback as traceback " \
                    "from celery_taskmeta a inner join test_celery_multi b on a.task_id=b.task_id;"
        cur.execute(query_str)
        rows = cur.fetchall()
        cur.close()
        return rows
